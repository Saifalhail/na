...
from .models import (
    User, UserProfile, DietaryRestriction, APIUsageLog,
    FoodItem, Meal, MealItem, MealAnalysis, NutritionalInfo,
    FavoriteMeal, NutritionData, RecipeIngredient
)
...
class UserProfileInline(admin.StackedInline):
    ...
    fields = (
        'gender', 'height', 'weight', 'activity_level',
        'daily_calorie_goal', 'daily_protein_goal', 'daily_carbs_goal', 'daily_fat_goal',
        'measurement_system', 'timezone', 'language', 'bio', 'avatar',
        'receive_email_notifications', 'receive_push_notifications',
        'show_nutritional_info_publicly', 'bmi', 'bmr', 'tdee'
    )
...
class UserAdmin(BaseUserAdmin):
    ...
    list_display = (
        ...
        'is_verified', 'account_type', 'is_staff', 'created_at'
    )
    list_filter = (
    ...
    fieldsets = (
        ...
        (_('Account status'), {
            'fields': (
                'is_verified', 'verification_token', 'account_type',
                'is_profile_public', 'last_login_ip'
            )
        }),
        (_('Permissions'), {
            'fields': (
    ...
    readonly_fields = ('verification_token', 'created_at', 'updated_at', 'last_login', 'date_joined')
...
class UserProfileAdmin(admin.ModelAdmin):
    ...
    list_display = (
        ...
        'activity_level', 'bmi', 'measurement_system'
    )
    ...
    fieldsets = (
        ...
        (_('Preferences'), {
            'fields': (
                'measurement_system', 'timezone', 'language',
                'receive_email_notifications', 'receive_push_notifications',
...
class DietaryRestrictionAdmin(admin.ModelAdmin):
    ...
    list_display = (
        'user', 'name', 'restriction_type', 'severity',
...
class APIUsageLogAdmin(admin.ModelAdmin):
    ...
    list_display = (
        'user', 'endpoint', 'method', 'response_status_code',
        'response_time_ms', 'ai_tokens_used', 'created_at'
    )
    list_filter = (
    ...
    fieldsets = (
        (_('Request Information'), {
            'fields': (
                'user', 'endpoint', 'method', 'ip_address',
                'user_agent', 'request_body_size'
            )
        }),
        (_('Response Information'), {
            'fields': (
                'response_status_code', 'response_time_ms',
                'ai_tokens_used', 'error_message'
            )
        }),
...
class MealItemInline(admin.TabularInline):
    ...
    model = MealItem
    extra = 1
    fields = ['food_item', 'quantity', 'unit', 'calories', 'protein', 'carbohydrates', 'fat']
...
class FoodItemAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'name', 'brand', 'calories', 'protein', 'carbohydrates', 'fat',
    ...
    fieldsets = [
        ...
        (_('Source Information'), {
            'fields': ['source', 'external_id', 'created_by']
        }),
        (_('Settings'), {
...
class MealAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'name', 'user', 'meal_type', 'consumed_at',
    ...
    readonly_fields = ['id', 'total_calories', 'total_macros', 'created_at', 'updated_at']
    
    fieldsets = [
        (_('Basic Information'), {
            'fields': ['id', 'user', 'name', 'meal_type']
        }),
        (_('Meal Details'), {
            'fields': ['consumed_at', 'image', 'notes']
        }),
        (_('Location'), {
...
class MealAnalysisAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'meal', 'ai_service', 'confidence_score',
        'analysis_time_ms', 'tokens_used', 'is_accurate', 'created_at'
    ]
    ...
    fieldsets = [
        ...
        (_('Analysis Details'), {
            'fields': [
                'ai_service', 'confidence_score',
                'analysis_time_ms', 'tokens_used'
            ]
        }),
        (_('User Feedback'), {
            'fields': ['is_accurate', 'user_notes']
        }),
        (_('AI Response'), {
            'fields': ['ai_response'],
            'classes': ['collapse']
...
class FavoriteMealAdmin(admin.ModelAdmin):
    ...
    fieldsets = [
        (_('Basic Information'), {
            'fields': ['user', 'meal', 'name']
        }),
        (_('Settings'), {
            'fields': ['is_template', 'quick_add_order']
        }),
        (_('Usage Statistics'), {
...
class NutritionDataAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'id', 'user', 'calories', 'protein', 'carbohydrates', 'fat', 'created_at'
    ]
