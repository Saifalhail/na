...
from api.models import UserProfile, Meal, FavoriteMeal
...
class UserMealsView(generics.ListAPIView):
    ...
    def get_queryset(self):
        user_id = self.kwargs.get('pk')
        return Meal.objects.filter(user_id=user_id).select_related('user').prefetch_related('meal_items__food_item')
...
class UserStatisticsView(APIView):
    ...
    def get(self, request, pk):
        try:
            ...
            meal_type_counts = meals.values('meal_type').annotate(count=Count('id')).order_by('-count')
            most_common_meal_type = meal_type_counts[0]['meal_type'] if meal_type_counts else 'none'
            
            # Meal type distribution
            meal_type_distribution = {item['meal_type']: item['count'] for item in meal_type_counts}
            
            # Calculate streak
            meal_dates = meals.values_list('consumed_at__date', flat=True).distinct().order_by('-consumed_at__date')
            streak_days = 0
            if meal_dates:
