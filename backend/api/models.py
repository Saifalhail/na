...
class User(AbstractUser):
    ...
    firebase_uid = models.CharField(
        ...
        null=True,
        unique=True,
        help_text=_("Firebase user identifier for authentication"),
    )
    ...
    social_avatar_url = models.URLField(
        _("social avatar URL"),
        blank=True,
        help_text=_("Profile picture URL from social providers"),
    )
...
class UserProfile(models.Model):
    ...
    receive_email_notifications = models.BooleanField(
        _("receive email notifications"), default=True
    )

    receive_push_notifications = models.BooleanField(
        _("receive push notifications"), default=True
    )
...
class DietaryRestriction(models.Model):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="dietary_restrictions"
    )
    ...
    def __str__(self):
        return (
            f"{self.user.email} - {self.name} ({self.get_restriction_type_display()})"
        )
...
class APIUsageLog(models.Model):
    ...
    ip_address = models.GenericIPAddressField(
        _("IP address"), help_text=_("IP address of the request")
    )

    user_agent = models.TextField(
        _("user agent"), blank=True, help_text=_("User agent string from the request")
    )
    ...
    response_status_code = models.PositiveIntegerField(
        _("response status code"), help_text=_("HTTP status code of the response")
    )
    ...
    def __str__(self):
        ...
        return (
            f"{user_str} - {self.method} {self.endpoint} - {self.response_status_code}"
        )
...
class FoodItem(models.Model):
    ...
    carbohydrates = models.DecimalField(
        _("carbohydrates (g)"), max_digits=8, decimal_places=2
    )
    fat = models.DecimalField(_("fat (g)"), max_digits=8, decimal_places=2)
    fiber = models.DecimalField(
        _("fiber (g)"), max_digits=8, decimal_places=2, default=0
    )
    sugar = models.DecimalField(
        _("sugar (g)"), max_digits=8, decimal_places=2, default=0
    )
    sodium = models.DecimalField(
        _("sodium (mg)"), max_digits=8, decimal_places=2, default=0
    )

    # Additional nutrients
    saturated_fat = models.DecimalField(
        _("saturated fat (g)"), max_digits=8, decimal_places=2, null=True, blank=True
    )
    trans_fat = models.DecimalField(
        _("trans fat (g)"), max_digits=8, decimal_places=2, null=True, blank=True
    )
    cholesterol = models.DecimalField(
        _("cholesterol (mg)"), max_digits=8, decimal_places=2, null=True, blank=True
    )
    potassium = models.DecimalField(
        _("potassium (mg)"), max_digits=8, decimal_places=2, null=True, blank=True
    )
    vitamin_a = models.DecimalField(
        _("vitamin A (%)"), max_digits=5, decimal_places=2, null=True, blank=True
    )
    vitamin_c = models.DecimalField(
        _("vitamin C (%)"), max_digits=5, decimal_places=2, null=True, blank=True
    )
    calcium = models.DecimalField(
        _("calcium (%)"), max_digits=5, decimal_places=2, null=True, blank=True
    )
    iron = models.DecimalField(
        _("iron (%)"), max_digits=5, decimal_places=2, null=True, blank=True
    )
    ...
    external_id = models.CharField(
        ...
        max_length=100,
        blank=True,
        help_text=_("ID from external database"),
    )
...
class Meal(models.Model):
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(
                fields=["user", "meal_type"]
            ),  # For filtering by meal type per user
...
class MealAnalysis(models.Model):
    ...
    ai_service = models.CharField(
        _("AI service"), max_length=20, choices=AI_SERVICE_CHOICES, default="gemini"
    )
    ...
    analysis_time_ms = models.PositiveIntegerField(
        _("analysis time (ms)"), help_text=_("Time taken for analysis in milliseconds")
    )
    tokens_used = models.PositiveIntegerField(
        _("tokens used"), default=0, help_text=_("Number of AI tokens consumed")
    )
    ...
    user_notes = models.TextField(
        _("user notes"), blank=True, help_text=_("User corrections or notes")
    )
    ...
    def recalculate_nutrition(self, new_servings):
        ...
        if self.servings_per_recipe and new_servings:
            ...
            recalculated = {
                ...
                "carbohydrates": (
                    float(self.carbohydrates * ratio) if self.carbohydrates else None
                ),
...
class NutritionalInfo(models.Model):
    ...
    vitamin_a_iu = models.DecimalField(
        _("vitamin A (IU)"), max_digits=10, decimal_places=2, null=True, blank=True
    )
    vitamin_a_rae = models.DecimalField(
        _("vitamin A (mcg RAE)"), max_digits=10, decimal_places=2, null=True, blank=True
    )
    vitamin_c = models.DecimalField(
        _("vitamin C (mg)"), max_digits=10, decimal_places=2, null=True, blank=True
    )
    vitamin_d = models.DecimalField(
        _("vitamin D (mcg)"), max_digits=10, decimal_places=2, null=True, blank=True
    )
    vitamin_e = models.DecimalField(
        _("vitamin E (mg)"), max_digits=10, decimal_places=2, null=True, blank=True
    )
    vitamin_k = models.DecimalField(
        _("vitamin K (mcg)"), max_digits=10, decimal_places=2, null=True, blank=True
    )

    # B Vitamins
    thiamin = models.DecimalField(
        _("thiamin (mg)"), max_digits=10, decimal_places=3, null=True, blank=True
    )
    riboflavin = models.DecimalField(
        _("riboflavin (mg)"), max_digits=10, decimal_places=3, null=True, blank=True
    )
    niacin = models.DecimalField(
        _("niacin (mg)"), max_digits=10, decimal_places=2, null=True, blank=True
    )
    vitamin_b6 = models.DecimalField(
        _("vitamin B6 (mg)"), max_digits=10, decimal_places=3, null=True, blank=True
    )
    folate = models.DecimalField(
        _("folate (mcg)"), max_digits=10, decimal_places=2, null=True, blank=True
    )
    vitamin_b12 = models.DecimalField(
        _("vitamin B12 (mcg)"), max_digits=10, decimal_places=2, null=True, blank=True
    )

    # Minerals
    calcium = models.DecimalField(
        _("calcium (mg)"), max_digits=10, decimal_places=2, null=True, blank=True
    )
    iron = models.DecimalField(
        _("iron (mg)"), max_digits=10, decimal_places=2, null=True, blank=True
    )
    magnesium = models.DecimalField(
        _("magnesium (mg)"), max_digits=10, decimal_places=2, null=True, blank=True
    )
    phosphorus = models.DecimalField(
        _("phosphorus (mg)"), max_digits=10, decimal_places=2, null=True, blank=True
    )
    potassium = models.DecimalField(
        _("potassium (mg)"), max_digits=10, decimal_places=2, null=True, blank=True
    )
    sodium = models.DecimalField(
        _("sodium (mg)"), max_digits=10, decimal_places=2, null=True, blank=True
    )
    zinc = models.DecimalField(
        _("zinc (mg)"), max_digits=10, decimal_places=2, null=True, blank=True
    )

    # Other nutrients
    omega_3 = models.DecimalField(
        _("omega-3 (g)"), max_digits=10, decimal_places=3, null=True, blank=True
    )
    omega_6 = models.DecimalField(
        _("omega-6 (g)"), max_digits=10, decimal_places=3, null=True, blank=True
    )
    water = models.DecimalField(
        _("water (g)"), max_digits=10, decimal_places=2, null=True, blank=True
    )
    caffeine = models.DecimalField(
        _("caffeine (mg)"), max_digits=10, decimal_places=2, null=True, blank=True
    )
    alcohol = models.DecimalField(
        _("alcohol (g)"), max_digits=10, decimal_places=2, null=True, blank=True
    )
...
class RecipeIngredient(models.Model):
    ...
    nutrition_data = models.ForeignKey(
        NutritionData, on_delete=models.CASCADE, related_name="ingredients"
    )
...
class TOTPDevice(models.Model):
    ...
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="totp_devices"
    )
    name = models.CharField(
        _("device name"),
        max_length=64,
    ...
    key = models.CharField(
        _("secret key"), max_length=80, help_text=_("Base32-encoded secret key")
    )
    ...
    step = models.PositiveSmallIntegerField(
        _("step"), default=30, help_text=_("Time step in seconds")
    )
...
class BackupCode(models.Model):
    ...
    used = models.BooleanField(
        _("used"), default=False, help_text=_("Whether this code has been used")
    )
    used_at = models.DateTimeField(
        _("used at"), null=True, blank=True, help_text=_("When this code was used")
    )
    created_at = models.DateTimeField(_("created at"), auto_now_add=True)
...
class Notification(models.Model):
    ...
    @property
    def is_read(self):
        """Check if notification is read (for backward compatibility)."""
...
class SubscriptionPlan(models.Model):
    ...
    plan_type = models.CharField(
        _("plan type"), max_length=20, choices=PLAN_TYPE_CHOICES, unique=True
    )
...
class Subscription(models.Model):
    ...
    def can_use_ai_analysis(self):
        ...
        if self.ai_analyses_reset_date and timezone.now() > self.ai_analyses_reset_date:
            ...
            self.ai_analyses_reset_date = timezone.now().replace(
                day=1
...
class PaymentMethod(models.Model):
    ...
    stripe_payment_method_id = models.CharField(
        _("Stripe payment method ID"), max_length=255, unique=True
    )
...
class Payment(models.Model):
    ...
    stripe_invoice_id = models.CharField(
        _("Stripe invoice ID"), max_length=255, blank=True
    )
    ...
    refund_amount = models.DecimalField(
        _("refund amount"), max_digits=10, decimal_places=2, default=0.00
    )
...
class Invoice(models.Model):
    ...
    # Stripe integration
    stripe_invoice_id = models.CharField(
        _("Stripe invoice ID"), max_length=255, unique=True, blank=True
    )
    ...
    tax_amount = models.DecimalField(
        _("tax amount"), max_digits=10, decimal_places=2, default=0.00
    )
    total_amount = models.DecimalField(
        _("total amount"), max_digits=10, decimal_places=2
    )
    amount_paid = models.DecimalField(
        _("amount paid"), max_digits=10, decimal_places=2, default=0.00
    )
    currency = models.CharField(_("currency"), max_length=3, default="USD")
    ...
    def __str__(self):
        return (
            f"Invoice {self.invoice_number} - {self.user.email} (${self.total_amount})"
        )
...
class DeviceToken(models.Model):
    ...
    device_id = models.CharField(
        _("device ID"), max_length=255, help_text=_("Unique device identifier")
    )
    ...
    app_version = models.CharField(
        _("app version"), max_length=50, null=True, blank=True
    )
...
class PushNotification(models.Model):
    ...
    expo_receipt_id = models.CharField(_("Expo receipt ID"), max_length=255, blank=True)
    expo_ticket_id = models.CharField(_("Expo ticket ID"), max_length=255, blank=True)
...
class SyncLog(models.Model):
    ...
    sync_type = models.CharField(
        _("sync type"), max_length=20, choices=SYNC_TYPE_CHOICES, default="incremental"
    )
    status = models.CharField(
        _("status"), max_length=20, choices=STATUS_CHOICES, default="started"
    )
    ...
    notifications_downloaded = models.IntegerField(
        _("notifications downloaded"), default=0
    )
...
class MalwareScanLog(models.Model):
    ...
    def __str__(self):
        ...
        return (
            f"{self.file_name} - {status} ({self.scan_date.strftime('%Y-%m-%d %H:%M')})"
        )


class SMSOTPCode(models.Model):
    """
    Model for storing SMS OTP verification codes.
    """
    ...
    ip_address = models.GenericIPAddressField(
        null=True, blank=True, help_text="IP address that requested the OTP"
    )
    ...
    def __str__(self):
        return f"{self.phone_number} - {self.otp_code} ({'used' if self.is_used else 'active'})"
    ...
    @classmethod
    def create_otp(cls, phone_number: str, ip_address: str = None) -> "SMSOTPCode":
        """Create a new OTP code for a phone number."""
        # Deactivate any existing OTP for this phone number
    ...
    def is_valid(self) -> bool:
        """Check if this OTP code is still valid."""
    ...
    def verify_otp(cls, phone_number: str, otp_code: str) -> bool:
        ...
        try:
            otp_obj = cls.objects.get(
                phone_number=phone_number, otp_code=otp_code, is_used=False
            )
...
class SMSRateLimit(models.Model):
    """Model for tracking SMS rate limits per phone number."""
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=["last_sms_sent"]),
        ]

    def __str__(self):
        return f"{self.phone_number} - {self.sms_count_today}/day, {self.sms_count_hour}/hour"

    @classmethod
    def check_rate_limit(cls, phone_number: str) -> dict:
        """Check if phone number has exceeded rate limits."""
        now = timezone.now()

        try:
            rate_limit = cls.objects.get(phone_number=phone_number)

            # Reset counters if needed
            if now >= rate_limit.daily_reset_at:
                rate_limit.sms_count_today = 0
                rate_limit.daily_reset_at = now.replace(
                    hour=0, minute=0, second=0, microsecond=0
                ) + timedelta(days=1)

            if now >= rate_limit.hourly_reset_at:
                rate_limit.sms_count_hour = 0
                rate_limit.hourly_reset_at = now.replace(
            ...
            rate_limit = cls.objects.create(
                ...
                + timedelta(days=1),
                hourly_reset_at=now.replace(minute=0, second=0, microsecond=0)
                + timedelta(hours=1),
            )

        # Check limits (configurable via settings)
        daily_limit = getattr(settings, "SMS_DAILY_LIMIT", 10)
        hourly_limit = getattr(settings, "SMS_HOURLY_LIMIT", 3)

        return {
            "allowed": (
                rate_limit.sms_count_today < daily_limit
                and rate_limit.sms_count_hour < hourly_limit
            ),
            "daily_remaining": max(0, daily_limit - rate_limit.sms_count_today),
            "hourly_remaining": max(0, hourly_limit - rate_limit.sms_count_hour),
        }
...
class BlacklistedToken(models.Model):
    ...
    Provides persistent token blacklisting that survives cache clears.
    """

    jti = models.CharField(
        max_length=255, unique=True, help_text="JWT ID (jti) claim from the token"
    )
    ...
    ip_address = models.GenericIPAddressField(
        null=True, blank=True, help_text="IP address that initiated the blacklisting"
    )
    ...
    def blacklist_all_user_tokens(
        cls, user, reason: str = "logout_all", ip_address: str = None
    ) -> int:
        """
        ...
        """
        # This would require getting all outstanding tokens for the user
        # For now, we'll create a "global blacklist" entry
        # In practice, you'd integrate with your token storage/tracking
        from rest_framework_simplejwt.tokens import RefreshToken

        # Create a special entry to invalidate all tokens issued before now
        invalidation_token = cls.objects.create(
            jti=f"invalidate_all_{user.id}_{timezone.now().timestamp()}",
            user=user,
            token_type="invalidation",
            expires_at=timezone.now() + timedelta(days=365),  # Long expiry
            reason=reason,
            ip_address=ip_address,
        )

        # Also update user's last logout time for additional security
        if hasattr(user, "last_logout_at"):
