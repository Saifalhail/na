...
from api.views.payments import (
    SubscriptionPlanListView, UserSubscriptionListView, CreateSubscriptionView,
    SubscriptionDetailView, CancelSubscriptionView, PaymentMethodListView,
    AddPaymentMethodView, RemovePaymentMethodView, PaymentHistoryView,
    InvoiceListView, InvoiceDetailView, BillingHistoryView, StripeWebhookView,
    current_subscription, ai_usage_check, record_ai_usage
)
...
urlpatterns = [
    ...
    path('subscriptions/<uuid:pk>/cancel/', CancelSubscriptionView.as_view(), name='cancel-subscription'),
    
    # Payment methods
    path('payment-methods/', PaymentMethodListView.as_view(), name='payment-methods'),
    path('payment-methods/add/', AddPaymentMethodView.as_view(), name='add-payment-method'),
    path('payment-methods/<uuid:pk>/remove/', RemovePaymentMethodView.as_view(), name='remove-payment-method'),
