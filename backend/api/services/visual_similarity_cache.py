...
class VisualSimilarityCache:
    ...
    def __init__(self):
        ...
        self.similarity_threshold = getattr(settings, 'VISUAL_SIMILARITY_THRESHOLD', 0.75)
    ...
    def extract_visual_features(self, image_data: bytes) -> Optional[ImageFeatures]:
        """
        Extract visual features from a food image.
    ...
    def _calculate_color_histogram(self, image: Image.Image, bins: int = 8) -> List[float]:
    ...
    def _extract_texture_features(self, image: Image.Image) -> List[float]:
        ...
        try:
            ...
            texture_complexity = abs(original_stat.stddev[0] - blur_stat.stddev[0]) / 128.0
    ...
    def calculate_similarity(self, features1: ImageFeatures, features2: ImageFeatures) -> float:
        """
        ...
        try:
            ...
            color_sim = self._color_similarity(features1.dominant_colors, features2.dominant_colors)
            similarities.append(('color', color_sim, 0.3))
            
            # Histogram similarity
            hist_sim = self._histogram_similarity(features1.color_histogram, features2.color_histogram)
            ...
            # Texture similarity
            texture_sim = self._texture_similarity(features1.texture_features, features2.texture_features)
            ...
            logger.debug(f"Similarity calculation: {dict((name, f'{sim:.3f}') for name, sim, _ in similarities)} -> {final_similarity:.3f}")
    ...
    def _color_similarity(self, colors1: List[Tuple[int, int, int]], colors2: List[Tuple[int, int, int]]) -> float:
    ...
    def _texture_similarity(self, texture1: List[float], texture2: List[float]) -> float:
    ...
    def store_analysis(
        ...
        try:
            ...
            confidence = analysis_result.get('data', {}).get('confidence', {}).get('overall', 0)
            if confidence < self.min_confidence_to_cache:
                logger.debug(f"Skipping cache storage - confidence too low: {confidence}")
                return False
    ...
    def find_similar_analysis(
        ...
        try:
                ...
                try:
                    ...
                    if similarity >= threshold and similarity > best_similarity:
                        ...
                        if cached_analysis:
                            ...
                            if not context or cached_analysis['context_hash'] == context_hash:
                                best_match = cached_analysis
                                best_similarity = similarity
                            
                except Exception as e:
                    logger.warning(f"Error processing cached features {stored_hash}: {e}")
                    continue
            ...
            if best_match:
                ...
                logger.info(f"Visual similarity cache hit: {best_similarity:.3f} similarity")
