...
from api.models import (
    User, SubscriptionPlan, Subscription, Payment, PaymentMethod, Invoice
)
    ...
    def create_subscription(
        ...
        try:
            ...
            subscription = Subscription.objects.create(
                ...
                ai_analyses_reset_date=timezone.now().replace(day=1) + timedelta(days=32)
            )
    ...
    def _handle_invoice_payment_succeeded(self, invoice_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            Payment.objects.create(
                ...
                amount=Decimal(invoice_data.get('amount_paid', 0)) / 100,  # Convert from cents
            ...
            logger.info(f"Processed successful payment for subscription {subscription.id}")
    ...
    def _handle_invoice_payment_failed(self, invoice_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            Payment.objects.create(
                ...
                subscription=subscription,
                stripe_invoice_id=invoice_data.get('id'),
                amount=Decimal(invoice_data.get('amount_due', 0)) / 100,  # Convert from cents
                currency=invoice_data.get('currency', 'usd').upper(),
                status='failed',
                description=f"Failed payment for {subscription.plan.name}",
                failure_reason=invoice_data.get('last_payment_error', {}).get('message', 'Payment failed')
            )
            ...
            logger.warning(f"Processed failed payment for subscription {subscription.id}")
