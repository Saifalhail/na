...
This service handles sending SMS notifications to users and OTP codes.
"""
...
class SMSService:
    ...
    def send_sms(
        ...
        if len(message) > 1600:
            ...
            return {"success": False, "error": "Message too long", "sid": None}

        try:
            # Send SMS via Twilio
            kwargs = {"body": message, "from_": self.from_number, "to": to_number}
            ...
            return {"success": False, "error": "Unexpected error occurred", "sid": None}
    ...
    def format_phone_number(
        self, phone_number: str, country_code: str = "+1"
    ) -> Optional[str]:
        """
    ...
    def send_otp_code(
        self, phone_number: str, otp_code: str, ip_address: str = None
    ) -> Dict[str, Any]:
        """
        ...
        from api.models import SMSRateLimit

        # Check rate limits
        rate_limit_status = SMSRateLimit.check_rate_limit(phone_number)
        ...
        # Create SMS message
        message = (
            f"Your verification code is: {otp_code}. This code expires in 10 minutes."
        )

        # Send SMS
        result = self.send_sms(formatted_phone, message)

        if result["success"]:
            # Increment rate limit counter
            SMSRateLimit.increment_sms_count(formatted_phone)
            logger.info(f"OTP code sent successfully to {formatted_phone}")
        else:
            logger.error(
                f"Failed to send OTP to {formatted_phone}: {result.get('error')}"
            )
