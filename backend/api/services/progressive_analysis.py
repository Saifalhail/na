...
class ProgressiveAnalysisService:
    ...
    def __init__(self):
        ...
        self.session_timeout = getattr(settings, 'PROGRESSIVE_ANALYSIS_TIMEOUT', 300)  # 5 minutes
    ...
    def start_progressive_analysis(self, session_id: str) -> bool:
        ...
        try:
            ...
            future = self.executor.submit(self._execute_progressive_analysis, session_id)
    ...
    def _execute_progressive_analysis(self, session_id: str):
        ...
        try:
            ...
            self.stats['average_completion_time'] = (
                (self.stats['average_completion_time'] * (self.stats['sessions_completed'] - 1) + completion_time) /
            ...
            logger.info(f"Completed progressive analysis for session: {session_id} in {completion_time:.2f}s")
    ...
    def _execute_stage(self, session: AnalysisSession, stage_index: int, stage_config: Dict[str, Any]):
        ...
        completed_weight = sum(
            config['weight'] for i, config in enumerate(self.analysis_stages)
            if i <= stage_index
        )
    ...
    def _execute_cache_checking(self, session: AnalysisSession) -> Dict[str, Any]:
        ...
        try:
            ...
            if getattr(settings, 'AI_USE_VISUAL_CACHE', True):
                visual_match = visual_cache.find_similar_analysis(image_data, session.context)
    ...
    def _execute_ai_analysis(self, session: AnalysisSession) -> Dict[str, Any]:
        ...
        try:
            ...
            # Perform analysis
            analysis_result = gemini_service.analyze_food_image(image_data, session.context)
            ...
            return {
                'ai_success': True,
                'ingredients_found': len(analysis_result.get('data', {}).get('ingredients', [])),
                'confidence_score': analysis_result.get('data', {}).get('confidence', {}).get('overall', 0),
    ...
    def _execute_nutrition_calculation(self, session: AnalysisSession) -> Dict[str, Any]:
    ...
    def _handle_stage_error(self, session: AnalysisSession, stage_index: int, error_message: str):
        ...
        logger.error(f"Stage {stage.stage_id} failed for session {session.session_id}: {error_message}")
    ...
    def _get_session(self, session_id: str) -> Optional[AnalysisSession]:
        ...
        try:
            ...
            if session_data:
                ...
                stages = [AnalysisStage(**stage_data) for stage_data in session_data['stages']]
    ...
    def get_session_status(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            'final_result': session.final_result if session.status == 'completed' else None
        }
    ...
    def get_service_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'success_rate': (
                self.stats['sessions_completed'] / max(self.stats['sessions_created'], 1)
            ) * 100,
