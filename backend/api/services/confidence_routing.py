...
class ConfidenceRoutingService:
    ...
    def _save_performance_stats(self):
        ...
        try:
            ...
            cache.set(
                self.performance_cache_key, stats_dict, timeout=86400 * 7
            )  # 7 days
    ...
    def route_analysis_request(
        ...
        try:
            ...
            confidence_requirement = (
                confidence_requirement or self.default_confidence_threshold
            )
            ...
            best_model_id = max(
                model_scores.keys(), key=lambda k: model_scores[k]["total_score"]
            )
            ...
            fallback_models = [
                model_id
                for model_id, score_info in sorted(
                    model_scores.items(),
            ...
            estimated_time = self._estimate_response_time(
                best_model_id, complexity_score
            )
            ...
            logger.info(
                f"Routed to model {best_model_id}: {best_score_info['reasoning']}"
            )
            return decision
            ...
            default_model = (
                ...
                else list(self.available_models.keys())[0]
            )
            return RoutingDecision(
                ...
                confidence_threshold=confidence_requirement
                or self.default_confidence_threshold,
                fallback_models=[],
                routing_factors={},
            )

    def _estimate_image_complexity(
        self, image_data: bytes, context: Optional[Dict[str, Any]]
    ) -> float:
    ...
    def _score_models(
        self, routing_factors: Dict[str, float]
        ...
        for model_id, model_config in self.available_models.items():
            try:
                ...
                confidence_adjustment = (
                    model_config.confidence_boost - 1.0
                )  # Convert to adjustment
                scores["accuracy"] = min(
                    100, base_accuracy + (confidence_adjustment * 20)
                )

                if scores["accuracy"] >= routing_factors["confidence_requirement"]:
                    reasoning_parts.append(
                        f"meets confidence requirement ({scores['accuracy']:.1f})"
                    )
                else:
                    reasoning_parts.append(
                        f"below confidence requirement ({scores['accuracy']:.1f} < {routing_factors['confidence_requirement']})"
                    )
                ...
                if routing_factors["cost_limit"] > 0:
                    estimated_cost = (
                        self._estimate_token_usage(None, None)
                        * model_config.cost_per_token
                    )
                    if estimated_cost <= routing_factors["cost_limit"]:
                        scores["cost"] = (
                            1.0 - (estimated_cost / routing_factors["cost_limit"])
                        ) * 100
                        reasoning_parts.append(
                            f"within cost limit (${estimated_cost:.4f})"
                        )
                    else:
                        scores["cost"] = 0
                        reasoning_parts.append(
                            f"exceeds cost limit (${estimated_cost:.4f})"
                        )
                else:
                ...
                if performance.total_requests > 0:
                    ...
                    recency_bonus = max(
                        0, 20 - ((time.time() - performance.last_used) / 3600)
                    )  # Bonus for recent use

                    scores["performance"] = (
                        (success_rate * 50)
                        + (avg_confidence_score * 0.3)
                        + recency_bonus
                    )
                ...
                scores["complexity"] = complexity_handling[complexity_level][
                    model_config.accuracy_tier
                ]
    ...
    def _estimate_token_usage(
        self, image_data: Optional[bytes], context: Optional[Dict[str, Any]]
    ) -> int:
    ...
    def execute_routed_analysis(
        ...
        if self.enable_fallback_routing and routing_decision.fallback_models:
            logger.warning(
                f"Primary model {routing_decision.selected_model} failed, trying fallbacks"
            )
            ...
            for fallback_model in routing_decision.fallback_models:
                ...
                if fallback_result.get("success"):
                    ...
                    self._update_model_performance(
                        ...
                        success=True,
                        response_time=response_time,
                        confidence=fallback_result.get("data", {})
                        .get("confidence", {})
                        .get("overall", 0),
                        cost=routing_decision.estimated_cost
                        * 0.8,  # Estimate for fallback
                    )
    ...
    def _try_model_analysis(
        ...
        try:
            ...
            if model_id.startswith("gemini"):
                ...
                if result.get("success"):
                    confidence = (
                        result.get("data", {}).get("confidence", {}).get("overall", 0)
                    )
                    if confidence < confidence_threshold:
                        logger.warning(
                            f"Model {model_id} confidence {confidence} below threshold {confidence_threshold}"
                        )
                        return {
                            "success": False,
    ...
    def get_routing_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for model_id, performance in self.performance_stats.items():
                ...
                if model_config:
                    model_stats[model_id] = {
                        ...
                        "last_used_ago": (
                            (time.time() - performance.last_used) / 3600
                            if performance.last_used > 0
                            else None
                        ),
                    }
            ...
            return {
                ...
                "total_requests": sum(
                    perf.total_requests for perf in self.performance_stats.values()
                ),
    ...
    def _calculate_model_health_score(self, performance: ModelPerformance) -> float:
        ...
        recency_score = max(
            0, 1.0 - ((time.time() - performance.last_used) / (86400 * 7))
        )  # Decay over 7 days

        health_score = (
            (success_rate * 50) + (confidence_score * 30) + (recency_score * 20)
        )
    ...
    def _calculate_overall_success_rate(self) -> float:
        ...
        total_successes = sum(
            perf.successful_requests for perf in self.performance_stats.values()
        )
