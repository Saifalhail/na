...
from ..utils.cache_lock import CacheStampedeProtection, with_cache_lock
...
class IngredientCache:
    ...
    def __init__(self):
        ...
        self.max_combinations = getattr(
            settings, "INGREDIENT_COMBINATION_MAX_ENTRIES", 500
        )
        self.default_ttl = getattr(
            settings, "INGREDIENT_CACHE_TTL", 86400 * 30
        )  # 30 days
        self.similarity_threshold = getattr(
            settings, "INGREDIENT_SIMILARITY_THRESHOLD", 0.8
        )
        self.min_confidence_to_cache = getattr(
    ...
    def find_ingredient(self, name: str, unit: str = "g") -> Optional[CachedIngredient]:
        """
    ...
    def _find_fuzzy_ingredient_match(
        self, search_key: IngredientKey
    ) -> Optional[CachedIngredient]:
        """Find fuzzy matches for an ingredient with cache stampede protection."""
        try:
            # Use cache stampede protection for expensive fuzzy matching
            cache_key = f"fuzzy_search:{search_key.base_name}:{search_key.category}"
            
            def perform_fuzzy_search():
                return self._perform_fuzzy_search_operation(search_key)
            
            return CacheStampedeProtection.get_or_set_with_lock(
                cache_key=cache_key,
                generator_func=perform_fuzzy_search,
                timeout=300,  # 5 minutes
                lock_timeout=60,  # 1 minute lock
                stale_timeout=1800  # 30 minutes stale data
            )
    ...
    def _perform_fuzzy_search_operation(self, search_key: IngredientKey) -> Optional[CachedIngredient]:
        """Perform the actual expensive fuzzy search operation."""
        ...
        for cache_key, stored_key_dict in ingredient_index.items():
            try:
                ...
                logger.warning(
                    f"Error processing cached ingredient {cache_key}: {e}"
                )
                continue
    ...
    def _calculate_ingredient_similarity(
        self, key1: IngredientKey, key2: IngredientKey
    ) -> float:
        ...
        try:
            ...
            for ingredient in ingredients:
                normalized_key = self.normalize_ingredient(
                    ingredient.get("name", ""), ingredient.get("unit", "g")
                )
            ...
            combination_str = json.dumps(
                {"ingredients": normalized_ingredients, "serving_size": serving_size},
                sort_keys=True,
            )
            ...
            confidence = (
                nutrition_result.get("data", {}).get("confidence", {}).get("overall", 0)
            )
            if confidence < self.min_confidence_to_cache:
                return False
            ...
            combination = IngredientCombination(
                ...
                nutrition_per_serving=nutrition_result.get("data", {}).get(
                    "per_serving", {}
                ),
            ...
            logger.debug(
                f"Stored ingredient combination: {len(normalized_ingredients)} ingredients"
            )
            return True
    ...
    def find_combination(
        self, ingredients: List[Dict[str, Any]], serving_size: int = 1
    ...
    def _normalize_nutrition_data(
        self, nutrition_data: Dict[str, float], unit: str
    ) -> NutritionData:
    ...
    def _update_combination_index(self, combination_hash: str, ingredients: List[str]):
        ...
        try:
            ...
            cache.set(
                self.combination_index_key,
                combination_index,
                timeout=self.default_ttl * 2,
            )
