...
def deprecated(
    ...
    def decorator(view_func):
        ...
        def wrapper(*args, **kwargs):
            ...
            endpoint = (
                f"{request.method} {request.path}" if request else "Unknown endpoint"
            )
            logger.warning(
            ...
            if isinstance(response, (HttpResponse, Response)):
                _add_deprecation_headers(
                    response, version_removed, alternative, date_removed, reason
                )
...
def warn_deprecated_parameter(
    parameter_name: str, version_removed: str, alternative: str = None
):
    """
