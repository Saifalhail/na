...
class MobileCacheManager:
    ...
    def cache_dashboard_data(user_id: int, data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            cache.set(cache_key, cached_data, MobileCacheManager.DASHBOARD_CACHE_TIMEOUT)
    ...
    def get_dashboard_data(user_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if cached_data and isinstance(cached_data, dict):
                ...
                if timezone.now() - cached_at < timedelta(seconds=MobileCacheManager.DASHBOARD_CACHE_TIMEOUT):
    ...
    @staticmethod
    def cache_meal_list(user_id: int, meals_data: List[Dict], filters: Dict = None) -> bool:
        """
        ...
        try:
            ...
            cache.set(cache_key, cached_data, MobileCacheManager.MEAL_LIST_CACHE_TIMEOUT)
    ...
    def cache_user_profile(user_id: int, profile_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            cache.set(cache_key, cached_data, MobileCacheManager.USER_PROFILE_CACHE_TIMEOUT)
        ...
        for cache_type in cache_types:
            try:
                ...
                if cache_type == 'meals':
                    ...
                    for filter_variation in ['', 'breakfast', 'lunch', 'dinner', 'recent']:
                ...
                logger.error(f"Failed to invalidate {cache_type} cache for user {user_id}: {e}")
        
        logger.info(f"Invalidated {invalidated} cache keys for user {user_id}")
        return invalidated
    
    @staticmethod
    def cache_sync_data(user_id: int, sync_data: Dict[str, Any], last_sync: str = None) -> bool:
        """
        ...
        try:
            ...
            cache.set(cache_key, cached_data, MobileCacheManager.SYNC_DATA_CACHE_TIMEOUT)
...
class MobileQueryOptimizer:
    ...
    def get_user_subscription_info(user):
        ...
        active_subscription = user.subscriptions.select_related('plan').filter(
            status__in=['active', 'trialing']
        ).first()
        
        subscription_info = {
            'is_active': bool(active_subscription),
            'plan_name': active_subscription.plan.name if active_subscription else 'Free',
            'plan_type': active_subscription.plan.plan_type if active_subscription else 'free',
