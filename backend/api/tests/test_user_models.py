...
from api.tests.factories import (DietaryRestrictionFactory, UserFactory,
                                 UserProfileFactory)
...
class UserModelTest(TestCase):
    ...
    def test_user_factory_with_profile(self):
        ...
        )  # date_of_birth is on User model
...
class DietaryRestrictionModelTest(TestCase):
    ...
    def test_unique_constraint(self):
        ...
        DietaryRestriction.objects.create(
            user=self.user, name="Gluten", restriction_type="intolerance"
        )
    ...
    def test_string_representation(self):
        ...
        restriction = DietaryRestriction.objects.create(
            user=self.user, name="Dairy", restriction_type="intolerance"
        )
    ...
    def test_restriction_type_choices(self):
        ...
        restriction = DietaryRestriction.objects.create(user=self.user, name="Test")
    ...
    def test_multiple_restrictions_per_user(self):
        ...
        for i, name in enumerate(["Peanuts", "Gluten", "Dairy"]):
            restriction = DietaryRestriction.objects.create(
                user=self.user, name=name, restriction_type="allergy"
            )
            restrictions.append(restriction)

        self.assertEqual(self.user.dietary_restrictions.count(), 3)
        self.assertEqual(list(self.user.dietary_restrictions.all()), restrictions)

    def test_optional_fields(self):
        """Test that optional fields can be blank."""
        restriction = DietaryRestriction.objects.create(
            user=self.user, name="Vegetarian", restriction_type="preference"
        )
