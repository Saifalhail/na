...
from api.models import UserProfile, Meal, FoodItem, MealItem, FavoriteMeal
from api.factories import UserFactory, UserProfileFactory, MealFactory, FoodItemFactory
...
class UserViewsTestCase(TestCase):
    def setUp(self):
        ...
        self.user1 = UserFactory(first_name="John", last_name="Doe", email="john@example.com")
        self.profile1 = UserProfileFactory(user=self.user1)
        
        self.user2 = UserFactory(first_name="Jane", last_name="Smith", email="jane@example.com")
        ...
        self.meal3 = MealFactory(user=self.user1, consumed_at=timezone.now() - timedelta(days=10))
    ...
    def test_user_list_filtering(self):
        ...
        response = self.client.get(reverse('api:users:user-list'), {'is_active': 'true'})
        self.assertEqual(len(response.data['results']), 2)
        
        response = self.client.get(reverse('api:users:user-list'), {'is_active': 'false'})
        ...
        response = self.client.get(reverse('api:users:user-list'), {'account_type': 'premium'})
        self.assertEqual(len(response.data['results']), 1)
