...
class TestMealViewSet:
    ...
    def test_create_meal_with_items(self):
        ...
        data = {
            ...
            'meal_items': [
                {
                    'food_item_id': str(self.food1.id),
                    'quantity': 100,
                    'unit': 'g'
                },
                {
                    'food_item_id': str(self.food2.id),
                    'quantity': 150,
                    'unit': 'g'
                }
            ]
    ...
    def test_favorite_meal(self):
        ...
        assert response.status_code == status.HTTP_201_CREATED
        assert FavoriteMeal.objects.filter(
            user=self.user,
            meal=self.meal1
        ).exists()
    ...
    def test_unfavorite_meal(self):
        ...
        assert response.status_code == status.HTTP_204_NO_CONTENT
        assert not FavoriteMeal.objects.filter(
            user=self.user,
            meal=self.meal1
        ).exists()
