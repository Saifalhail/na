...
from api.security.validators import (
    validate_input, sanitize_html, add_rel_nofollow,
    validate_image_upload, validate_email, validate_password_strength,
    validate_sql_injection, validate_xss, SecureFileUploadSerializer,
    SanitizedCharField, SanitizedEmailField, SecurePasswordField,
...
class APIKeyManagerTestCase(TestCase):
    ...
    def test_get_cipher_generate_new_key(self, mock_fernet_class):
        ...
        with patch('api.security.api_keys.logger') as mock_logger:
            ...
            self.assertIn('Generated temporary key', mock_logger.warning.call_args[0][0])
    ...
    def test_get_api_key_from_cache(self):
        ...
        with patch.object(APIKeyManager, '_get_cipher') as mock_get_cipher:
            ...
            # Store key in cache
            cache.set('api_key_test_service', {
    ...
    def test_rotate_api_key(self):
        ...
        with patch.object(APIKeyManager, 'get_api_key_info') as mock_get_info, \
             patch.object(APIKeyManager, 'store_api_key') as mock_store, \
             patch('api.security.api_keys.logger') as mock_logger:
