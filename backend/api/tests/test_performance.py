...
from api.factories import UserFactory, MealFactory, MealItemFactory, FoodItemFactory
...
class DatabasePerformanceTest(TransactionTestCase):
    ...
    def setUp(self):
        ...
        for i in range(50):
            ...
            for j in range(3 + (i % 3)):
                MealItemFactory(
                    meal=meal,
                    food_item=self.food_items[j % len(self.food_items)]
                )
    ...
    def test_meal_list_query_count(self):
        ...
        with self.assertNumQueries(3):  # Should be a small, consistent number
            meals = Meal.objects.filter(user=self.user).prefetch_related(
                'meal_items',
                'favorited_by'
            ).annotate(
    ...
    def test_meal_detail_query_count(self):
        ...
        with self.assertNumQueries(4):  # Should be a small, consistent number
            meal_detail = Meal.objects.prefetch_related(
                'meal_items__food_item',
                'favorited_by'
            ).select_related('analysis').get(id=meal.id)
