...
from api.models import Notification, Meal, MealItem, FoodItem
from api.tests.factories import (
    UserFactory, NotificationFactory, MealFactory, 
    MealItemFactory, FoodItemFactory, UserProfileFactory
)
...
class SendEmailNotificationTestCase(TestCase):
    ...
    def test_send_email_notification_success(self, mock_logger, mock_render, mock_send_mail):
    ...
    @patch('api.tasks.email_tasks.send_mail')
    @patch('api.tasks.email_tasks.render_to_string')
    def test_send_email_notification_template_selection(self, mock_render, mock_send_mail):
...
class SendWelcomeEmailTestCase(TestCase):
    ...
    def test_send_welcome_email_failure(self, mock_send_mail):
        ...
        with patch('api.tasks.email_tasks.logger') as mock_logger:
            ...
            mock_logger.error.assert_called_once()
            self.assertIn('Failed to send welcome email', mock_logger.error.call_args[0][0])
...
class SendDailyNutritionSummaryTestCase(TestCase):
    ...
    def test_send_daily_nutrition_summary_with_meals(self, mock_logger, mock_send_email):
        ...
        MealItemFactory(meal=meal, food_item=food_item, quantity=150, calories=300, protein=15)
...
class SendWeeklyProgressReportTestCase(TestCase):
    ...
    def test_send_weekly_progress_report_with_meals(self, mock_logger, mock_send_email):
        ...
        self.assertEqual(notification.data['total_calories'], 2000.0)  # 5 meals × 400 calories
        
        # Check goal adherence calculation
        expected_adherence = (2000 / (2000 * 7)) * 100  # About 14.3%
        self.assertAlmostEqual(notification.data['goal_adherence'], expected_adherence, places=1)
...
class SendMealRemindersTestCase(TestCase):
    ...
    def test_send_meal_reminders_at_reminder_time(self, mock_logger, mock_timezone):
        ...
        mock_now = timezone.now().replace(hour=12, minute=2)  # Within 5 minutes of 12:00
    ...
    def test_send_meal_reminders_recent_meal_logged(self, mock_timezone):
        ...
        recent_meal = MealFactory(
            user=self.user,
            consumed_at=mock_now - timedelta(hours=1)
        )
...
class CheckAndNotifyAchievementsTestCase(TestCase):
    ...
    def test_check_daily_calorie_goal_achievement(self, mock_logger):
        ...
        food_item = FoodItemFactory(calories_per_100g=200)
        meal = MealFactory(user=self.user, consumed_at=today_start + timedelta(hours=12))
...
class CleanupOldNotificationsTestCase(TestCase):
    ...
    def test_cleanup_preserves_recent_notifications(self):
        ...
        self.assertTrue(
            Notification.objects.filter(id=recent_read.id).exists()
        )
        self.assertTrue(
            Notification.objects.filter(id=recent_sent.id).exists()
        )
        self.assertTrue(
            Notification.objects.filter(id=recent_pending.id).exists()
        )
