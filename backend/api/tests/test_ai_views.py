...
from api.models import FoodItem, Meal, MealAnalysis, MealItem
from api.tests.factories import FoodItemFactory, MealFactory, UserFactory
...
class TestAnalyzeImageView:
    ...
    def test_analyze_image_success(self, mock_analyze):
        ...
        data = {"image": image, "meal_type": "lunch", "notes": "Test meal"}
    ...
    def test_analyze_image_ai_service_error(self, mock_analyze):
        ...
        mock_analyze.return_value = {"success": False, "error": "API quota exceeded"}
    ...
    def test_analyze_image_mock_response(self):
        ...
        with patch("django.conf.settings.GEMINI_API_KEY", "your-gemini-api-key"):
            image = self.create_test_image()
            data = {"image": image, "meal_type": "breakfast"}
    ...
    def test_analyze_large_image(self):
        ...
        with patch(
            "api.services.gemini_service.GeminiService.analyze_food_image"
        ) as mock:
            mock.return_value = {
                "success": True,
                "data": {
...
class TestRecalculateNutritionView:
    ...
    def setup(self):
        ...
        self.food1 = FoodItemFactory(
            name="Chicken", calories=165, protein=31, carbohydrates=0, fat=3.6
        )
        self.food2 = FoodItemFactory(
            name="Rice", calories=130, protein=2.7, carbohydrates=28, fat=0.3
        )

    @patch(
        "api.services.gemini_service.GeminiService.calculate_nutrition_from_ingredients"
    )
    def test_recalculate_success(self, mock_calculate):
        ...
        assert (
            response.data["daily_values_percentage"]["calories"] == 12.5
        )  # 250/2000 * 100
    ...
    def test_recalculate_with_meal_update(self, mock_calculate):
        ...
        mock_calculate.return_value = {
            ...
            "data": {
                ...
                "ingredients_breakdown": [
                    {"calories": 250, "protein": 30, "carbohydrates": 0, "fat": 5},
                    {"calories": 150, "protein": 5, "carbohydrates": 40, "fat": 3},
                ],
            },
        }
    ...
    def test_recalculate_no_auth(self):
        ...
        data = {"ingredients": [{"name": "Chicken", "quantity": 100, "unit": "g"}]}
    ...
    def test_recalculate_invalid_data(self):
        ...
        data = {"ingredients": [{"name": "Chicken"}]}  # Missing quantity
        response = self.client.post(self.url, data, format="json")
        assert response.status_code == status.HTTP_400_BAD_REQUEST
    ...
    def test_recalculate_ai_service_error(self, mock_calculate):
        """Test handling of AI service errors."""
        mock_calculate.return_value = {"success": False, "error": "Rate limit exceeded"}
