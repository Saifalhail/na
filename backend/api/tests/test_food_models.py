...
class MealModelTest(TestCase):
    ...
    def test_meal_type_choices(self):
        ...
        meal = Meal.objects.create(
            user=self.user, name="Test Meal", consumed_at=datetime.now()
        )
    ...
    def test_string_representation(self):
        ...
        meal = Meal.objects.create(
            user=self.user, name="Lunch", consumed_at=consumed_time
        )
    ...
    def test_meal_ordering(self):
        ...
        meal1 = Meal.objects.create(
            user=self.user, name="Breakfast", consumed_at=datetime(2024, 1, 1, 8, 0)
        )
        meal2 = Meal.objects.create(
            user=self.user, name="Lunch", consumed_at=datetime(2024, 1, 1, 12, 0)
        )
        meal3 = Meal.objects.create(
            user=self.user, name="Dinner", consumed_at=datetime(2024, 1, 1, 18, 0)
        )
    ...
    def test_user_meals_relationship(self):
        ...
        for i in range(3):
            meal = Meal.objects.create(
                user=self.user, name=f"Meal {i+1}", consumed_at=datetime.now()
            )
            meals.append(meal)
...
class MealItemModelTest(TestCase):
    ...
    def test_unit_conversion(self):
        ...
        for unit, quantity, expected_grams in units_and_quantities:
            meal_item = MealItem.objects.create(
                meal=self.meal, food_item=self.food_item, quantity=quantity, unit=unit
            )
    ...
    def test_string_representation(self):
        """Test string representation."""
        meal_item = MealItem.objects.create(
            meal=self.meal, food_item=self.food_item, quantity=Decimal("150"), unit="g"
        )
    ...
    def test_quantity_validation(self):
        ...
        meal_item = MealItem(
            meal=self.meal, food_item=self.food_item, quantity=Decimal("-10"), unit="g"
        )
    ...
    def test_meal_cascade_delete(self):
        ...
        meal_item = MealItem.objects.create(
            meal=self.meal, food_item=self.food_item, quantity=Decimal("100"), unit="g"
        )
...
class MealAnalysisModelTest(TestCase):
    ...
    def test_one_to_one_relationship(self):
        ...
        analysis = MealAnalysis.objects.create(
            meal=self.meal, ai_service="gemini", ai_response={}, analysis_time_ms=1000
        )
