...
class ErrorDetailSerializer(serializers.Serializer):
    ...
    field = serializers.CharField(required=False, help_text="Field name that caused the error")
...
class RateLimitErrorResponseSerializer(ErrorResponseSerializer):
    ...
    retry_after = serializers.IntegerField(
        required=False,
        help_text="Number of seconds to wait before retrying"
    )
    limit = serializers.IntegerField(
        required=False,
        help_text="Rate limit for this endpoint"
    )
    remaining = serializers.IntegerField(
        required=False,
        help_text="Number of requests remaining in current window"
    )
...
class AuthenticationErrorResponseSerializer(ErrorResponseSerializer):
    ...
    www_authenticate = serializers.CharField(
        required=False,
        help_text="WWW-Authenticate header value"
    )
...
class PermissionErrorResponseSerializer(ErrorResponseSerializer):
    ...
    required_permission = serializers.CharField(
        required=False,
        help_text="Permission required for this action"
    )
    required_account_type = serializers.CharField(
        required=False,
        help_text="Account type required for this feature"
    )
...
ERROR_CODES = {
    ...
    'INSUFFICIENT_PERMISSIONS': {
        ...
        'example': {
            ...
            'request_id': 'req_abc123def456',
            'details': {
                'required_permission': 'meals.delete_meal'
            }
        }
    },
