...
class SubscriptionPlanSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        if instance.plan_type == "premium":
            features.extend(["Priority support", "Advanced analytics", "Export data"])
...
class SubscriptionSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        if instance.plan.ai_analysis_limit > 0:
            usage_percentage = (
                instance.ai_analyses_used / instance.plan.ai_analysis_limit
            ) * 100
...
class PaymentMethodSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        if instance.payment_type == "card":
            data["display_name"] = (
                f"{instance.card_brand.title()} ending in {instance.card_last4}"
            )
            ...
            if instance.card_exp_month and instance.card_exp_year:
                ...
                if instance.card_exp_year < today.year or (
                    instance.card_exp_year == today.year
                    and instance.card_exp_month < today.month
                ):
...
class CancelSubscriptionSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if not attrs.get("cancel_at_period_end", True) and not attrs.get(
            "cancellation_reason"
        ):
            raise serializers.ValidationError(
