...
class EnableTwoFactorSerializer(serializers.Serializer):
    ...
    def create(self, validated_data):
        ...
        device = TOTPDevice.objects.create(
            user=user, name=validated_data["device_name"], key=secret, confirmed=False
        )
...
class QRCodeSerializer(serializers.Serializer):
    ...
    def validate_device_id(self, value):
        ...
        try:
            ...
            raise serializers.ValidationError(
                "Invalid device ID or device already confirmed."
            )
        return value
    ...
    def get_qr_code(self):
        ...
        provisioning_uri = totp.provisioning_uri(
            name=user.email,
            issuer_name=(
                settings.SITE_NAME if hasattr(settings, "SITE_NAME") else "Nutrition AI"
            ),
        )
...
class LoginWithTwoFactorSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if token and backup_code:
            raise serializers.ValidationError(
                "Provide either token or backup code, not both."
            )
        ...
        if token:
            ...
            try:
                code = BackupCode.objects.get(
                    user=user, code=backup_code.upper(), used=False
                )
