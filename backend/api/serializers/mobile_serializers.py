...
class SendPushNotificationSerializer(serializers.Serializer):
    ...
    ttl = serializers.IntegerField(
        min_value=0, max_value=2419200, default=86400, required=False
    )  # Max 28 days
    priority = serializers.ChoiceField(
        choices=["normal", "high"], default="normal", required=False
    )
...
class BatchOperationSerializer(serializers.Serializer):
    ...
    operations = serializers.ListField(
        child=serializers.DictField(), min_length=1, max_length=50  # Limit batch size
    )
    ...
    def validate_operations(self, value):
        ...
        for i, operation in enumerate(value):
            ...
            if "local_id" not in operation:
                raise serializers.ValidationError(
                    f"Operation {i} missing 'local_id' field."
                )
...
class ImageOptimizationSerializer(serializers.Serializer):
    ...
    quality = serializers.IntegerField(
        min_value=1, max_value=100, default=85, help_text="Compression quality (1-100)"
    )
    max_width = serializers.IntegerField(
        min_value=100, max_value=4096, default=1024, help_text="Maximum width in pixels"
    )
