...
class AnalyzeImageView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            log_api_usage(
                ...
                ai_tokens_used=len(analysis_result["data"].get("ingredients", []))
                * 100,  # Rough estimate
            )
    ...
    def _get_geographic_context(self, latitude: float, longitude: float) -> Dict[str, str]:
        """Get geographic context based on coordinates."""
        # Simple geographic regions based on latitude
        if latitude > 66.5:
            region = "arctic"
        elif latitude > 23.5:
            region = "northern_temperate"
        elif latitude > -23.5:
            region = "tropical"
        elif latitude > -66.5:
            region = "southern_temperate"
        else:
            region = "antarctic"
    ...
    def _assess_lighting_quality(self, brightness: float, contrast: float = None) -> str:
        """Assess lighting quality for better AI analysis."""
        if brightness < 0.2:
            return "very_dark"
        elif brightness < 0.4:
            return "dark"
    ...
    def _create_meal_from_analysis(
        self, user, image, analysis_data: Dict, validated_data: Dict
    ) -> Meal:
    ...
    def _get_or_create_food_item(self, ingredient_data: Dict, user) -> FoodItem:
    ...
    def _mock_response(self, image, validated_data: Dict) -> Response:
        ...
        mock_response = {
            ...
            "suggestions": ["Image quality is good", "All items clearly visible"],
        }
...
class RecalculateNutritionView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            nutrition_result = gemini_service.calculate_nutrition_from_ingredients(
                ingredient_list, serving_size=serving_size
            )
            ...
            if meal_id:
                try:
                    ...
                    meal = self._update_meal_with_recalculation(
                        meal, ingredients, nutrition_data
                    )
                except Meal.DoesNotExist:
                    logger.warning(
                        f"Meal {meal_id} not found for user {request.user.id}"
                    )

            # Prepare response
            response_data = {
                "total_nutrition": nutrition_data["nutrition"],
                "per_serving": nutrition_data.get(
                    "per_serving", nutrition_data["nutrition"]
                ),
                "ingredients_breakdown": nutrition_data.get(
    ...
    def _update_meal_with_recalculation(
        self, meal: Meal, ingredients: List[Dict], nutrition_data: Dict
    ) -> Meal:
        ...
        with transaction.atomic():
            ...
            for idx, ingredient in enumerate(ingredients):
                ...
                ingredient_nutrition = (
                    nutrition_data.get("ingredients_breakdown", [])[idx]
                    if idx < len(nutrition_data.get("ingredients_breakdown", []))
                    else {}
                )
                ...
                food_item = FoodItem.objects.create(
                    ...
                    carbohydrates=Decimal(
                        str(ingredient_nutrition.get("carbohydrates", 0))
                    ),
    ...
    def _mock_recalculation(
        self, ingredients: List[Dict], serving_size: int
    ) -> Response:
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            response_data = {
                ...
                "configuration": {
                    "use_advanced_prompts": getattr(
                        settings, "AI_USE_ADVANCED_PROMPTS", True
                    ),
                    "use_visual_cache": getattr(settings, "AI_USE_VISUAL_CACHE", True),
                    "use_ingredient_cache": getattr(
                        settings, "AI_USE_INGREDIENT_CACHE", True
                    ),
                    "use_regular_cache": getattr(settings, "AI_USE_CACHE", True),
                    "visual_similarity_threshold": getattr(
                        settings, "VISUAL_SIMILARITY_THRESHOLD", 0.75
                    ),
                    "ingredient_similarity_threshold": getattr(
                        settings, "INGREDIENT_SIMILARITY_THRESHOLD", 0.8
                    ),
                    "min_confidence_to_cache": getattr(
                        settings, "MIN_CONFIDENCE_TO_CACHE", 70
                    ),
                    "min_ingredient_confidence_to_cache": getattr(
                        settings, "MIN_INGREDIENT_CONFIDENCE_TO_CACHE", 70
                    ),
                },
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if success:
                ...
                logger.warning(
                    f"Cache cleared by admin user {request.user.id}: {cache_type}"
                )
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            session_id = progressive_analysis_service.create_analysis_session(
                user_id=request.user.id, image_data=image_bytes, context=context
            )

            # Start the analysis
            analysis_started = progressive_analysis_service.start_progressive_analysis(
                session_id
            )
...
class ProgressiveAnalysisStatsView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            response_data = {
                ...
                "configuration": {
                    ...
                    "websocket_enabled": bool(
                        getattr(settings, "CHANNEL_LAYERS", None)
                    ),
            ...
            return Response(
                {
                    "error": "Failed to retrieve progressive analysis statistics",
                    "details": str(e),
                },
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if not analysis_result.get("success"):
                raise AIServiceError(
                    analysis_result.get(
                        "error", "Failed to analyze image with confidence routing"
                    )
                )
            ...
            with transaction.atomic():
                ...
                ai_response["routing_decision"] = (
                    asdict(routing_decision)
                    if hasattr(routing_decision, "__dict__")
            ...
            logger.error(
                f"AI service error in confidence routing: {str(e)}", exc_info=True
            )
            # Log failed API usage
            processing_time = (
                int((time.time() - start_time) * 1000)
            ...
            logger.error(
                f"Unexpected error in confidence routing analysis: {str(e)}",
                exc_info=True,
            )
            # Log failed API usage
            processing_time = (
        ...
        try:
            ...
            response_data = {
                ...
                "configuration": {
                    ...
                    "default_confidence_threshold": getattr(
                        settings, "DEFAULT_CONFIDENCE_THRESHOLD", 75.0
                    ),
                    "enable_cost_optimization": getattr(
                        settings, "ENABLE_COST_OPTIMIZATION", True
                    ),
                    "max_cost_per_request": getattr(
                        settings, "MAX_COST_PER_REQUEST", 0.10
                    ),
                    "enable_fallback_routing": getattr(
                        settings, "ENABLE_FALLBACK_ROUTING", True
                    ),
                },
                "timestamp": timezone.now().isoformat(),
            }
            ...
            return Response(
                {
                    "error": "Failed to retrieve confidence routing statistics",
                    "details": str(e),
                },
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
