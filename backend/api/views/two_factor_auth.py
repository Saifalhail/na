...
class CompleteTwoFactorLoginView(APIView):
    ...
    @extend_schema(
        ...
        responses={
            ...
            400: {"description": "Invalid 2FA credentials or session expired"},
        },
        tags=["Two-Factor Authentication"],
    )
    def post(self, request):
        """Complete 2FA login."""
        # SECURITY FIX: Get user ID from session, not from request body
        # This prevents attackers from completing 2FA for other users
        session_key = request.session.session_key
        if not session_key:
            return Response(
                {"error": "2FA session not found. Please login again."},
                status=status.HTTP_400_BAD_REQUEST,
            )

        # Check if there's a pending 2FA session using session key
        cache_key = f"2fa_pending_session_{session_key}"
        pending_data = cache.get(cache_key)

        if not pending_data:
            # Also check old-style cache key for backward compatibility during transition
            user_id = request.data.get("user_id")
            if user_id:
                old_cache_key = f"2fa_pending_{user_id}"
                pending_data = cache.get(old_cache_key)
                if pending_data:
                    # Migrate to new session-based caching
                    cache.set(cache_key, pending_data, timeout=300)  # 5 minutes
        ...
        # Get user ID from session data, not from request
        user_id = pending_data.get("user_id")
        if not user_id:
            return Response(
                {"error": "Invalid 2FA session data."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        refresh["account_type"] = (
            user.account_type if hasattr(user, "account_type") else "free"
        )
