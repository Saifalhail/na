...
class ProgressiveAnalysisConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        try:
            ...
            if isinstance(self.user, AnonymousUser):
                logger.warning("Anonymous user attempted to connect to progressive analysis WebSocket")
                await self.close()
                return
            ...
            logger.info(f"User {self.user.id} connected to progressive analysis WebSocket")
    ...
    async def disconnect(self, close_code):
        ...
        try:
            ...
            if hasattr(self, 'user'):
                logger.info(f"User {self.user.id} disconnected from progressive analysis WebSocket")
    ...
    async def receive(self, text_data):
        ...
        try:
            ...
            logger.error(f"Invalid JSON received in progressive analysis WebSocket: {e}")
            await self.send(text_data=json.dumps({
                'type': 'error',
