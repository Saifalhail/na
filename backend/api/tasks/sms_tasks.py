...
def send_sms_notification(self, notification_id):
    ...
    try:
        ...
        if not getattr(profile, "receive_sms_notifications", False):
            logger.info(
                f"User {notification.user.email} has SMS notifications disabled"
            )
        ...
        if not formatted_phone:
            logger.error(
                f"Invalid phone number for user {notification.user.email}: {phone_number}"
            )
            notification.status = "failed"
        ...
        if result["success"]:
            ...
            logger.info(
                f"SMS notification {notification_id} sent successfully to {formatted_phone}"
            )
            return {"success": True, "sid": result["sid"], "to": formatted_phone}
        else:
        ...
        logger.error(
            f"Unexpected error sending SMS notification {notification_id}: {str(e)}"
        )
...
def send_bulk_sms_notifications(notification_ids):
    ...
    for notification_id in notification_ids:
        try:
            ...
            results[notification_id] = {"task_id": result.id, "status": "queued"}
        except Exception as e:
            logger.error(f"Failed to queue SMS for notification {notification_id}: {e}")
            results[notification_id] = {"status": "error", "error": str(e)}
        ...
        if status:
            ...
            try:
                ...
                if notification:
                    ...
                    notification.metadata["sms_error_message"] = status.get(
                        "error_message"
                    )
                    ...
                    if status["status"] == "delivered":
                        ...
                        notification.status = "failed"
                        notification.error_message = status.get(
                            "error_message", "SMS delivery failed"
                        )

                    notification.save()
                    logger.info(
                        f"Updated notification status for SMS {message_sid}: {status['status']}"
                    )
