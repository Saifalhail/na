...
from api.models import Meal, Notification, UserProfile
    ...
    for user in users:
        try:
            ...
            meals = Meal.objects.filter(
                user=user, consumed_at__range=(today_start, today_end)
            ).aggregate(
                total_calories=Coalesce(
                    Sum("meal_items__calories"), Value(0), output_field=DecimalField()
                ),
                total_protein=Coalesce(
                    Sum("meal_items__protein"), Value(0), output_field=DecimalField()
                ),
                total_carbs=Coalesce(
                    Sum("meal_items__carbohydrates"),
                    Value(0),
                    output_field=DecimalField(),
                ),
                total_fat=Coalesce(
                    Sum("meal_items__fat"), Value(0), output_field=DecimalField()
                ),
                meal_count=Count("id"),
            )
    ...
    for user in users:
        try:
            ...
            week_stats = Meal.objects.filter(
                user=user, consumed_at__range=(week_start, now)
            ).aggregate(
                avg_calories=Coalesce(
                    Avg("meal_items__calories"), Value(0), output_field=DecimalField()
                ),
                total_calories=Coalesce(
                    Sum("meal_items__calories"), Value(0), output_field=DecimalField()
                ),
            ...
            if week_stats["meal_count"] > 0:
                ...
                adherence = (
                    (week_stats["total_calories"] / expected_calories * 100)
...
def send_meal_reminders():
    ...
    users = (
        UserProfile.objects.filter(
            user__is_active=True, receive_push_notifications=True
        )
        .exclude(meal_reminder_times=[])
        .select_related("user")
    )
                ...
                try:
                    ...
                    if hour == current_hour and abs(minute - current_minute) <= 5:
                        ...
                        recent_meal = Meal.objects.filter(
                            user=profile.user, consumed_at__gte=now - timedelta(hours=2)
                        ).exists()
                        ...
                        if not recent_meal:
                            ...
                            logger.info(
                                f"Meal reminder created for {profile.user.email} at {reminder_time}"
                            )

                except ValueError:
                    logger.error(
                        f"Invalid reminder time format for user {profile.user.email}: {reminder_time}"
                    )

        except Exception as e:
            logger.error(
                f"Failed to process meal reminders for {profile.user.email}: {str(e)}"
            )
...
def check_and_notify_achievements(user_id):
    ...
    try:
        ...
        today_meals = Meal.objects.filter(
            user=user, consumed_at__gte=today_start
        ).aggregate(
            total_calories=Coalesce(
                Sum("meal_items__calories"), Value(0), output_field=DecimalField()
            ),
        ...
        if profile.daily_calorie_goal and today_meals["total_calories"]:
            goal_percentage = (
                today_meals["total_calories"] / profile.daily_calorie_goal
            ) * 100
        ...
        # Optimized streak calculation - get all meal dates at once
        consecutive_days = 0
        check_date = now.date()

        # Get all distinct meal dates for the user in the last year (max reasonable streak)
        year_ago = now.date() - timedelta(days=365)
        meal_dates = set(
            Meal.objects.filter(
                user=user,
                consumed_at__date__gte=year_ago,
                consumed_at__date__lte=check_date,
            )
            .values_list("consumed_at__date", flat=True)
            .distinct()
        )
...
def cleanup_old_notifications():
    ...
    deleted_count = Notification.objects.filter(
        status="read", created_at__lt=cutoff_date
    ).delete()[0]
    ...
    archived_count = Notification.objects.filter(
        status="sent", created_at__lt=cutoff_date
